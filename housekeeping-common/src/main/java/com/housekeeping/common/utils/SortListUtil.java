...
public class SortListUtil<E> {
     ...
     * @param order   desc降序  null升序
     */
    public void SortByFloat(List<E> list, final String method, final String order) {
        Collections.sort(list, new Comparator<E>() {
            @Override
            public int compare(E o1, E o2) {
                int ret = 0;
                try {
                    ...
                    //倒序
                    if (StringUtils.isNotEmpty(order) && "desc".equalsIgnoreCase(order)) {
                        ret = ((Float)method2.invoke(o2, null)).compareTo(((Float)method1.invoke(o1, null)));
                    }
                    else {
                        ret = ((Float)method1.invoke(o1, null)).compareTo(((Float)method2.invoke(o2, null)));
                    }
                } catch (NoSuchMethodException e) {
                    e.printStackTrace();
                } catch (IllegalAccessException e) {
