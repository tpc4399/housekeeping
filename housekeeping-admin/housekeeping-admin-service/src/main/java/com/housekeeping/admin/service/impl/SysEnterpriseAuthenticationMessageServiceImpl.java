...
public class SysEnterpriseAuthenticationMessageServiceImpl
    ...
    public Boolean isValidate() {
        ...
        return companyDetails.getIsValidate();
    }
    ...
    public R sendAuthMessage(SysEnterpriseAuthenticationMessagePostDTO authMessageDTO) {
        ...
        if (authMessageDTO.getIsSend()){
            ...
            baseMapper.updateCompany(TokenUtils.getCurrentUserId(),authMessageDTO.getCompanyNumber());
     ...
     */
    @Override
    public R query(Page page,Integer status ) {
        QueryWrapper<SysEnterpriseAuthenticationMessage> queryWrapper = new QueryWrapper<>();
        if(status!=null){
            queryWrapper.eq("audit_status", status);
        }
        queryWrapper.orderByDesc("create_time");
        List<SysEnterpriseAuthenticationMessage> list = this.list(queryWrapper);
        Iterator<SysEnterpriseAuthenticationMessage> iterator = list.iterator();
        while (iterator.hasNext()) {
            SysEnterpriseAuthenticationMessage next = iterator.next();
    ...
    @Override
    public R sendSms(String prefixPhone, String phone) throws Exception {
        //生成随机验证码
